...
export const Dialog = ({ onClose, onSucess, type, children }: Props) => {
  ...
  return (
    <div
      ...
      onClick={close}
    >
      <div className="absolute max-w-[500px] min-w-[300px] min-h-[200px] text-center p-10 -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg left-1/2 top-1/2 sm:p-5">
        <div>{children}</div>
        {type === "Confirm" ? (
